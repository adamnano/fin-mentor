...
export default function ESGScoreChart({ params }: Props) {
  const [env, setEnv] = useState(params.environmental);
  const [soc, setSoc] = useState(params.social);
  const [gov, setGov] = useState(params.governance);
  ...
  const scoreColor = overallScore >= 70 ? "#4ade80" : overallScore >= 50 ? "#fb923c" : "#E63C3A";
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="text-right">
          <div className="text-xl font-bold" style={{ color: scoreColor }}>
            {overallScore}
            <span className="text-xs text-neutral-600 ml-1">/ 100</span>
          </div>
      ...
      <ResponsiveContainer width="100%" height={148}>
        <RadarChart data={data} margin={{ top: 5, right: 20, left: 20, bottom: 5 }}>
          ...
          <Radar name="Company" dataKey="company" stroke="#E63C3A" fill="#E63C3A" fillOpacity={0.2} strokeWidth={2} />
